# Section CNN - Trousers and Jeans - Model 2 - All
# Author Jose Smith
# Start Date: 20210121
# End Date: 20210204
### Changed all the Model Directories to Model_2_All
...
# Logging setup info
### Added HP_FILTER_SIZE_2 ###
with tf.summary.create_file_writer(r'logs/Model_2_All/hparam_tuning/').as_default():
    hp.hparams_config(
...
### Added HP_FILTER_SIZE_2 ###
    ...
    log_dir = "logs\\Model_2_All\\fit\\" + "run-{}".format(session_num)
...
### Changed the class names ###
    ...
    model.save(r"saved_models\Model_2_All\Run-{}".format(session_num))
...
for filter_size_1 in HP_FILTER_SIZE_1.domain.values:
    for filter_num in HP_FILTER_NUM.domain.values:
        for filter_size_2 in HP_FILTER_SIZE_2.domain.values:
            ...
            run('Logs/Model_2_All/hparam_tuning/' + run_name, hparams, session_num)

            session_num += 1

# Loading a model to evaluate on the test set
print('=============Loading a model to evaluate on the test set====================')
model = tf.keras.models.load_model(r"saved_models\Model_2_All\Run-1")
...
os.system('python -m tensorboard.main --logdir {}\\logs\\Model_2_All\\hparam_tuning'.format(osdir))
os.system('taskkill /im tensorboard.exe /f')
os.system('del /q %TMP%\.tensorboard-info\*')
os.system('python -m tensorboard.main --logdir {}\\logs\\Model_2_All\\fit'.format(osdir))
