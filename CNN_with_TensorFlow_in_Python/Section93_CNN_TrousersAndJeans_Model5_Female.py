# Section CNN - Trousers and Jeans - Model 5 - Female
# Author Jose Smith
# Start Date: 20210128
# End Date: 
...
data_train = np.load(r"CNN_with_TensorFlow_in_Python/9_Practical_Project/Dataset/Trousers & Jeans - Female - Train.npz")
data_val = np.load(r"CNN_with_TensorFlow_in_Python/9_Practical_Project/Dataset/Trousers & Jeans - Female - Validation.npz")
data_test = np.load(r"CNN_with_TensorFlow_in_Python/9_Practical_Project/Dataset/Trousers & Jeans - Female - Test.npz")
...
# Logging setup info
### Added HP_DENSE_SIZE ###
with tf.summary.create_file_writer(r'logs/Model_5_Female/hparam_tuning/').as_default():
    hp.hparams_config(
...
### Added HP_DENSE_SIZE ###
    ...
    log_dir = "logs\\Model_5_Female\\fit\\" + "run-{}".format(session_num)
    ...
    model.save(r"saved_models\Model_5_Female\Run-{}".format(session_num))
    ...
    for filter_num in HP_FILTER_NUM.domain.values:
        for filter_size_2 in HP_FILTER_SIZE_2.domain.values:
            for dense_size in HP_DENSE_SIZE.domain.values:
                ...
                run('Logs/Model_5_Female/hparam_tuning/' + run_name, hparams, session_num)

                session_num += 1

# Loading a model to evaluate on the test set
print('=============Loading a model to evaluate on the test set====================')
model = tf.keras.models.load_model(r"saved_models\Model_5_Female\Run-1")
...
os.system('python -m tensorboard.main --logdir {}\\logs\\Model_5_Female\\hparam_tuning'.format(osdir))
...
os.system('python -m tensorboard.main --logdir {}\\logs\\Model_5_Female\\fit'.format(osdir))
