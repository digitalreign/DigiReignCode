...
# Logging setup info
### Added HP_DENSE_SIZE ###
with tf.summary.create_file_writer(r'logs/Model_4/hparam_tuning/').as_default():
    hp.hparams_config(
...
### Added HP_DENSE_SIZE ###
    ...
    log_dir = "logs\\Model_4\\fit\\" + "run-{}".format(session_num)
...
### Changed the class names ###
    ...
    model.save(r"saved_models\Model_4\Run-{}".format(session_num))
    ...
    for filter_num in HP_FILTER_NUM.domain.values:
        for filter_size_2 in HP_FILTER_SIZE_2.domain.values:
            for dense_size in HP_DENSE_SIZE.domain.values:
                ...
                run('Logs/Model_4/hparam_tuning/' + run_name, hparams, session_num)

                session_num += 1

# Loading a model to evaluate on the test set
print('=============Loading a model to evaluate on the test set====================')
model = tf.keras.models.load_model(r"saved_models\Model_4\Run-1")
...
os.system('python -m tensorboard.main --logdir {}\\logs\\Model_4\\hparam_tuning'.format(osdir))
os.system('taskkill /im tensorboard.exe /f')
os.system('del /q %TMP%\.tensorboard-info\*')
os.system('python -m tensorboard.main --logdir {}\\logs\\Model_4\\fit'.format(osdir))
